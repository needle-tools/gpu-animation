...
void skin(inout float4 vert, inout float3 normal, inout float4 tangent, float4x4 m1, float w1, float4x4 m2, float w2, float4x4 m3, float w3, float4x4 m4, float w4)
{
...
void skin(inout float4 vert, inout float3 normal, inout float4 tangent, float4x4 m1, float w1, float4x4 m2, float w2, float4x4 m3)
{
...
void skin(inout float4 vert, inout float3 normal, inout float4 tangent, float4x4 m1, float w1, float4x4 m2)
{
...
void skin(inout float4 vert, inout float3 normal, inout float4 tangent, float4x4 mat)
{
    vert = mul(mat, vert);
    normal = mul(mat, float4(normal,0)).xyz;
    tangent = mul(mat, tangent);
...
void skin(inout float4 vert, inout float3 normal, inout float4 tangent, int vertId,
	sampler2D weights, float4 weightsTexel, sampler2D animation, float4 animationTexel, uint startIndex, uint animationLength, uint frame)
{
	...
	skin(vert, normal, tangent, m0, w0, m1);
	...
	skin(vert, normal, tangent, m0, w0, m1, w1, m2);
	...
	skin(vert, normal, tangent, m0, w0, m1, w1, m2, w2, m3, w3);
	#endif
}
